...
class App extends React.Component {
  ...
  fetchData = async () =>   {
    ...
    try {
      ...
      this.setState({weather: weatherData.daily}); //set weather as weatherdata.daily
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div>
      ...
      <ul className="weather">
        {dates.map((dates, i) => <Day date={dates} max={max.at(i)} min={min.at(i)} code={codes.at(i)} key={dates} isToday={i === 0} />)}
      </ul>
    </div>
  ) 
}
